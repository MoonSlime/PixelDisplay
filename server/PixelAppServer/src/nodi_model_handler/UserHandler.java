...
public class UserHandler {
    ...
    public ResponseModel loginUser(String accountName, HttpSession session) throws Exception {
        boolean isExistAccount = db.exist(
                "SELECT * FROM NODI_USER WHERE accountName=?",
                accountName
        );

        int affectedRowsNum = db.update(
                isExistAccount ? "UPDATE NODI_USER SET lastLoginTime=now() WHERE accountName=?" : "INSERT INTO NODI_USER (accountName, lastLoginTime ,joinTime) VALUES (?, now(), now())", // 존재 할 경우==>로그인 시간 최신화 / 아닐 경우==>계정 등록
                accountName
        );

        if (affectedRowsNum == 0) {
            return new ResponseModel(201, "num of affected rows is zero");
        }
    ...
    */
    public ResponseModel update(UserModel model) throws Exception {
        ...
        int affectedRowsNum = db.update(
                "UPDATE NODI_USER SET sex=?, age=? WHERE accountName=?",
                model.getSex(),
                model.getAge(),
    ...
    */
    public ResponseModel deleteUser(UserModel model) throws Exception {
        int affectedRowsNum = db.update(
                "DELETE FROM NODI_USER WHERE accountName=?",
                model.getAccountName()
        );
